...
shinyServer(function(input,output){
  ...
  table2<-reactive({
    ...
    BODYFAT<-lm1$coefficients[1]+as.numeric(table2$WEIGHT)*lm1$coefficients[2]*meas()[1]+as.numeric(table2$ABDOMEN)*lm1$coefficients[3]*meas()[2]+as.numeric(table2$WRIST)*lm1$coefficients[4]**meas()[3]
    table2<-cbind(BODYFAT,table2)
    return(table2)})
